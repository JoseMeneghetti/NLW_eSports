...
export function CreateAdModal({ games }: Props) {
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    const gameId = games.find((game) => game.title === selectedGame)?.id;
      ...
      <Dialog.Content className="fixed bg-[#2A2643] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
        ...
        <form className="mt-8 flex flex-col gap-4" onSubmit={handleCreateAd}>
          ...
          <footer className="mt-4 flex justify-end gap-4">
            <Dialog.Close className="bg-zinc-500 text-white px-5 h-12 rounded-md font-semibold hover:bg-zinc-600">
              Cancelar
            </Dialog.Close>
            <button
